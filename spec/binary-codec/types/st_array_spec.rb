...
RSpec.describe BinaryCodec::STArray do
  ...
  describe 'STArray' do
    ...
    it 'decodes a full transaction with STArray correctly' do
      ...
      # STObject#to_json hands back a Hash, so the whole object can be
      # compared rather than three fields out of it.
      decoded_json = st_object_class.from_hex(fixture["binary"]).to_json

      expect(decoded_json).to eq(fixture["json"])
    end
  end
end
